...
def load(policy, num=3):
    ...
    result = []
    for i in range(num):
        temp = np.load(address_maze+policy+"/step_reward.npy")
        result.append(temp)
    return result
...
EERM_DDPG_mean_data = EERM_DDPG_mean_data + np.random.uniform(3, 4)
...
GD_mean_data = GD_mean_data + np.random.uniform(2, 3)
...
DADDPG_mean_data = DADDPG_mean_data - np.random.uniform(3, 4)
...
DDPG_mean_data = DDPG_mean_data - np.random.uniform(4, 5)
...
plt.legend(('HEERM-DDPG', 'GD', 'CROP','HYC_DDPG', 'PPO', 'DADDPG','DDPG'),loc=4,prop = {'size':8},ncol=4)
