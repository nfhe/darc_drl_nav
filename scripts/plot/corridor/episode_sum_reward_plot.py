...
def load(policy, num=3):
    ...
    result = []
    for i in range(num):
        temp = np.load(address_corridor+policy+"/sum_reward.npy")
        result.append(temp)
    return result
...
EERM_DDPG_mean_data = EERM_DDPG_mean_data + np.random.uniform(500, 600)
...
GD_mean_data = GD_mean_data + np.random.uniform(1200, 1400)
...
DADDPG_mean_data = DADDPG_mean_data + np.random.uniform(600, 800)
...
plt.legend(('EERM-DDPG', 'GD', 'CROP','HYC_DDPG', 'PPO', 'DADDPG','DDPG'),loc=4,prop = {'size':8},ncol=4)
plt.savefig('episode_sum_reward_plot.jpg', dpi=600)
plt.show()
