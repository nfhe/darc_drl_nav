...
class GameState:
    def __init__(self):
        ...
        self.pub_target_point = rospy.Publisher('/target_point', Point, queue_size=10)
        ...
        self.modelPath =r'/home/he/catkin_nav/src/turtlebot3_ddpg_collision_avoidance/turtlebot_ddpg/model/model.sdf'
    ...
    def reset(self):
        ...
        if self.target_y<-11.5:
            self.target_y = -11.5
        # self.target_x = (np.random.random()-0.5)*5 + 10*index_x
        # self.target_y = (np.random.random()-0.5)*5 + 10*index_y
        self.target_x = 12*index_x
        self.target_y = 0
        self.target_point.x = self.target_x
        self.target_point.y = self.target_y
        ...
        # print("self.target_point:",self.target_point)
        time.sleep(1)
        self.pub.publish(self.move_cmd)
        self.rate.sleep()
    ...
    def read_action(self):
        ...
        # print ("action_value linear x is %s", action_value_total.linear.x,"action_value angular z is %s", action_value_total.angular.z)
        self.return_action[0][0] = action_value_total.angular.z
        self.return_action[0][1] = action_value_total.linear.x
        if self.return_action[0][1] > 0.26:
            self.return_action[0][1] = 0.26
    ...
    def read_game_step(self, time_step=0.1, linear_x=0.8, angular_z=0.3):
        ...
        # print("laser_crashed_reward:",self.laser_crashed_reward)
        self.laser_reward = sum(normalized_laser)-24
        self.collision_reward = self.laser_crashed_reward + self.laser_reward
        # self.dis_robot_goal_reward = - distance_turtlebot_target *1.0
